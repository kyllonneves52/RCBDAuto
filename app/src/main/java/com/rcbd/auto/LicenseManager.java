...
public class LicenseManager {
    ...
    private static final String BUILD_PERMITIDO = "QP1A.190711.020";
    ...
    public static boolean estaAtivado(Context context){
        ...
        try{
            ...
            // 6. Toda vez que abrir o app ele confere se Build.ID continua o mesmo
            ...
            if(!buildAtual.equals(buildGuardado)){
                LogManager.registar(context, "BUILD mudou. Bloqueado");
                return false;
            }

            long dataFinal = Long.parseLong(pegar(dados,"DATA_FINAL"));
            if(System.currentTimeMillis() >= dataFinal){
                LogManager.registar(context, "Licença expirada");
                return false;
            }
            return true;
        }catch(Exception e){
    ...
    public static boolean ativar(Context context, String chave){
        try{
            // 1. Pega Build.ID do celular -> SP1A.210812.017
            String buildTelefone = android.os.Build.ID.toUpperCase(Locale.ROOT);
            LogManager.registar(context, "BUILD telefone: " + buildTelefone);

            // 2. Compara com BUILD_PERMITIDO -> Tem que ser igual
            if(!buildTelefone.equals(BUILD_PERMITIDO)){
                LogManager.registar(context, "Falhou: BUILD diferente");
                return false;
            }
            ...
            // 3. Lê a chave -> RCBD-DF7A2792-3D-AB92
            String[] partes = chave.split("-");
            if(partes.length!= 4) return false;
            if(!partes[0].equals("RCBD")) return false;

            String id = partes[1];
            String dias = partes[2].replace("D","");
            String codigo = partes[3];

            // 4. Confere ID FIXO, DIAS e CODIGO
            if(!id.equals(ID_LICENCA)){
                LogManager.registar(context, "Falhou ID");
                return false;
            }
            if(!dias.equals(String.valueOf(DIAS_PLANO))){
                LogManager.registar(context, "Falhou dias");
                return false;
            }
            if(!codigo.equals(CODIGO)){
                LogManager.registar(context, "Falhou codigo");
                return false;
            }

            // 5. Salva tudo em /storage/emulated/0/Android/license.sys
            long inicio = System.currentTimeMillis();
            long finalizacao = inicio + (24L * 60 * 60 * 1000); // 1 DIA
            ...
            LogManager.registar(context, "Erro licença: "+e.getMessage());
            return false;
        }
    }
    ...
    public static String tempoRestante(Context context){
        try{
            ...
            long dias = restante / 86400000;
            long horas = (restante % 86400000) / 3600000;
            long minutos = (restante % 3600000) / 60000;
